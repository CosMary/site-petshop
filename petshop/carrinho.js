// --- Lista completa de produtos do catálogo ---
const productList = [
    ...
    { id: 8, title: "Bifinho de Carne", price: 29.90, image: "https://www.petlove.com.br/images/products/209356/product/Petisco_Nestl%C3%A9_Purina_Doguitos_Bifinho_de_Carne_para_C%C3%A3es_31267420-1_1.jpg?1627670983" },
    ...
    { id: 10, title: "Areia Sanitária para Gatos", price: 26.99, image: "https://m.magazineluiza.com.br/a-static/420x420/areia-higienica-caixa-de-areia-biocat-fina-natural-10-kg/123wepet1/5726/df3d6c04be11ad8e28512713e82d995b.jpeg" },
    { id: 11, title: "Ivermectina", price: 99.99, image: "https://m.media-amazon.com/images/I/61q-kuPZxZL.jpg" },
...
// --- Função para carregar os produtos automaticamente ---
...
// --- Verifica quando a página estiver carregada ---
...
function ready() {
    renderProducts(); // <--- Gera os produtos automaticamente
...
// --- Função para inspecionar o produto ---
function inspectProduct(event) {
    ...
    if (productDetail) {
        alert(`
Inspecionando: ${productDetail.title}.
Preço: R$${productDetail.price.toFixed(2).replace('.', ',')}.
Redirecionando para a página de detalhes do produto ID: ${productId}.
        `);
    } else {
...
function addProductToCart(event) {
    ...
    if (!product) {
        console.error("Produto não encontrado na lista: ID " + productId);
        return;
    }
    ...
    if (tableBody) {
        ...
        console.error("ERRO: O elemento 'tbody' do carrinho não foi encontrado. Verifique seu HTML.");
    }
}
...
function updateTotal() {
    ...
    for (var i = 0; i < cartProducts.length; i++) {
        ...
        if (productPriceElement) {
            productPrice = parseFloat(productPriceElement.value);
        } else {
            const priceText = cartProducts[i].getElementsByClassName("cart-product-price")[0]?.innerText;
            if (priceText) {
                productPrice = parseFloat(priceText.replace("R$", "").replace(",", "."));
            }
        }
    ...
    if (totalSpan) {
        totalSpan.innerText = "R$" + totalAmount;
    }
}
